...
#include <UbunOrSilicon_inferencing.h> //see in /lib folder
  ...
  if (countObj == 2) {
    ...
    for (size_t ix = 0; ix < result.bounding_boxes_count; ix++) {
      ...
      if (ind2[0] == -1) {
        ...
        if (bb.y < result.bounding_boxes[ind2[0]].y) {
          ind2[1] = ind2[0];
          ind2[0] = ix;
        } else {
          ind2[1] = ix;
    ...
    ESP_LOGI(TAG, "Order from Top to bottom is %s and then %s", result.bounding_boxes[ind2[0]].label, result.bounding_boxes[ind2[1]].label);
  }
}